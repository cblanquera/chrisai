...
## Sprint And Release Inputs

Sprint, release, and other `.agents/` views are assembled from specs,
references, reviews, source references, and active progress state. Before
producing a human-readable `.agents/` view, check that the required source
layers exist and call out missing information instead of inventing scope,
delivery state, or commitments.
...
If a human-readable `.agents/` view introduces new requirements, decisions, risks,
